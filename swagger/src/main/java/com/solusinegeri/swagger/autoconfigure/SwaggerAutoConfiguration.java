...
/*    */ public class SwaggerAutoConfiguration
  ...
  public OpenAPI openAPI(@Autowired(required = false) BuildProperties buildProperties) {
    return (new OpenAPI()).addSecurityItem((new SecurityRequirement()).addList("auth", Collections.emptyList())).components((new Components()).addSecuritySchemes("auth", (new SecurityScheme()).scheme("bearer").type(SecurityScheme.Type.HTTP))).info((new Info()).title(this.getApplicationName(buildProperties)).version(this.getApplicationVersion(buildProperties)));
  }
